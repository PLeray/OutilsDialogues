...
class StepBlockApp:
    ...
    def create_buttons(self):
        ...
        tk.Button(self.button_frame, text="Add Step", command=self.add_etape).pack(side=tk.LEFT, padx=5, pady=5)
        #tk.Button(self.button_frame, text="Ajouter Bloc", command=self.add_block).pack(side=tk.LEFT, padx=5, pady=5)
        tk.Button(self.button_frame, text="Connect Blocks", command=self.create_connections).pack(side=tk.LEFT, padx=5, pady=5)
        tk.Button(self.button_frame, text="Generate HTML Page", command=self.generate_project_html).pack(side=tk.LEFT, padx=5, pady=5)
        tk.Button(self.button_frame, text="Generate ogg files", command=self.generate_Ogg).pack(side=tk.LEFT, padx=5, pady=5)
        tk.Button(self.button_frame, text="See Html", command=self.open_project_web_page).pack(side=tk.LEFT, padx=5, pady=5)
    ...
    def import_playlist_to_block(self):
        if self.selected_block:
            ...
            try:
                ...
                print(f"Updated block: playlist_link={leBloc.playlist_lien}, comment={leBloc.comment}, title={leBloc.title}")
